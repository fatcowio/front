...
import { _userMembers } from './_user';
...
export const _groupList = [...Array(15)].map((_, index) => ({
  id: _mock.id(index),
  name: _mock.text.groupName(index % 8),
  description: _mock.text.description(index).slice(0, 60),
  cover: _mock.text.groupCover(index),
  members: _userMembers.map((member) => member),
  status: ['popular', 'new', ''][Math.round(Math.random() * 1000) % 3],
}));
