...
export default function EventPostCard({ post, index }: Props) {
  ...
  return (
    <Card>
      ...
      <PostContent id={id} title={title} view={view} attendees={share} createdAt={createdAt} />
    </Card>
  );
}
...
export function PostContent({ id, title, view, attendees, createdAt, index }: PostContentProps) {
  const isDesktop = useResponsive('up', 'md');
  ...
  const POST_INFO = [
    ...
    { number: view, icon: 'eva:heart-fill' },
    { number: attendees, icon: 'eva:people-fill' },
  ];
